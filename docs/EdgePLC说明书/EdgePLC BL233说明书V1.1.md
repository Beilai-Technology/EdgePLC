# EdgePLC BL233说明书V1.1

![image](EdgePLC BL233说明书V1.1-images/image44.jpeg)
![image](EdgePLC BL233说明书V1.1-images/image44.jpeg)
![image](EdgePLC BL233说明书V1.1-images/image33.png)
...
在工业自动化向“边缘计算+实时控制”深度融合的转型背景下，传统PLC面临智能计算的瓶颈，而通用工业计算机又难以满足高实时性控制的需求。EdgePLC BL233系列工业边缘控制器应运而生，旨在打破IT与OT的融合壁垒，提供真正意义上的“边缘控算一体”解决方案。

BL233系列采用“高性能ARM处理器 + 分布式I/O扩展”的架构，以全志科技 T113-i双核 ARM Cortex-A7为核心 ，提供玄铁 C906 RISC-V + HiFi4 DSP 多核处理器架构，在保障毫秒级实时控制的同时，具备强大的本地数据处理能力。

EdgePLC BL233工业边缘控制器深度融合实时控制、边缘智能、协议转换、远程运维与二次开发等功能，支持IEC 61131-3标准编程环境（OpenPLC/NexPLC）、IGH EtherCAT硬实时主站，并可扩展多达32块N系列分布式I/O模块，覆盖DI/DO/AI/AO/温度采集等多种信号类型。软件层面基于Ubuntu 20.04系统，集成Docker、Node-RED、Python/C++开发环境，实现从数据采集、实时分析到智能决策的全流程闭环。
...
![image](EdgePLC BL233说明书V1.1-images/image11.png)
![image](EdgePLC BL233说明书V1.1-images/image98.png)

### 1.3技术参数

| 系统 | 型号 | T113-i：2*ARM Cortex-A7，1*玄铁C906 RISC-V(64bit） |
|---|---|---|
| 系统 | 处理器频率 | 1.2GHz |
| 系统 | 内存 | DDR3 256/512MByte，1GByte |
| 系统 | 存储 | eMMC 4/8G，NAND FLASH 256MByte |
| 电源 | 输入电压 | DC ～V |
| 电源 | 功耗 | 正常：280mA@12V（带4G模块），220mA@12V（不带4G模块） 最大：700mA@12V |
| 电源 | 反接防护 | 支持 |
...
![image](EdgePLC BL233说明书V1.1-images/image93.png)
![image](EdgePLC BL233说明书V1.1-images/image92.png)
...
。
![image](EdgePLC BL233说明书V1.1-images/image91.png)
...
|---|---|---|---|---|---|---|
| 端口号 | 1 | 2 | 3 | 4 | 5 | 6 |
| 名称 | ttyAS4-A | ttyAS4-B | GND | ttyAS3-A | ttyAS3-B | GND |
...
使用RS485串口时，将RS485线接至端口上（ttl转485），打开sscom5和mobaxterm对接收发，如RS485-2端口（x90模块），其设备文件为/dev/ttyAS3；设置其波特率设为 115200，8N1，无校验位。
...
![image](EdgePLC BL233说明书V1.1-images/image90.png)

串口助手发送数据

![image](EdgePLC BL233说明书V1.1-images/image89.png)

终端界面收到数据

![image](EdgePLC BL233说明书V1.1-images/image88.png)

按“Ctrl+C”停止。

然后更换x90另外一个ttyAS4口继续测试。

![image](EdgePLC BL233说明书V1.1-images/image87.png)

![image](EdgePLC BL233说明书V1.1-images/image86.png)

![image](EdgePLC BL233说明书V1.1-images/image85.png)

![image](EdgePLC BL233说明书V1.1-images/image84.png)

![image](EdgePLC BL233说明书V1.1-images/image83.png)
...
![image](EdgePLC BL233说明书V1.1-images/image82.png)
![image](EdgePLC BL233说明书V1.1-images/image81.png)

![image](EdgePLC BL233说明书V1.1-images/image80.png)
![image](EdgePLC BL233说明书V1.1-images/image79.png)

通过左边文件夹页面在文件夹下，把N板识别文件复制进去

![image](EdgePLC BL233说明书V1.1-images/image78.png)
![image](EdgePLC BL233说明书V1.1-images/image77.png)
...
![image](EdgePLC BL233说明书V1.1-images/image76.png)
![image](EdgePLC BL233说明书V1.1-images/image75.png)

![image](EdgePLC BL233说明书V1.1-images/image74.png)
![image](EdgePLC BL233说明书V1.1-images/image73.png)
...
![image](EdgePLC BL233说明书V1.1-images/image72.png)
![image](EdgePLC BL233说明书V1.1-images/image71.png)

root@bliiot:/# ion set 10 1 //通过address设置输出1

root@bliiot:/# ion get 10 //通过address查看

![image](EdgePLC BL233说明书V1.1-images/image70.png)
![image](EdgePLC BL233说明书V1.1-images/image69.png)
...
![image](EdgePLC BL233说明书V1.1-images/image68.png)
![image](EdgePLC BL233说明书V1.1-images/image67.png)
...
![image](EdgePLC BL233说明书V1.1-images/image66.png)
![image](EdgePLC BL233说明书V1.1-images/image65.png)

将DI（即为通道）短接

![image](EdgePLC BL233说明书V1.1-images/image64.png)
![image](EdgePLC BL233说明书V1.1-images/image63.png)

观察N板灯亮情况，对应DI短接形成回路灯亮。

![image](EdgePLC BL233说明书V1.1-images/image62.png)

![image](EdgePLC BL233说明书V1.1-images/image61.png)

![image](EdgePLC BL233说明书V1.1-images/image60.png)

![image](EdgePLC BL233说明书V1.1-images/image59.png)

![image](EdgePLC BL233说明书V1.1-images/image58.png)
![image](EdgePLC BL233说明书V1.1-images/image57.png)

![image](EdgePLC BL233说明书V1.1-images/image56.png)
![image](EdgePLC BL233说明书V1.1-images/image55.png)

![image](EdgePLC BL233说明书V1.1-images/image54.png)
![image](EdgePLC BL233说明书V1.1-images/image53.png)

![image](EdgePLC BL233说明书V1.1-images/image52.png)

![image](EdgePLC BL233说明书V1.1-images/image51.png)

![image](EdgePLC BL233说明书V1.1-images/image50.png)

![image](EdgePLC BL233说明书V1.1-images/image49.png)

![image](EdgePLC BL233说明书V1.1-images/image48.png)

![image](EdgePLC BL233说明书V1.1-images/image47.png)

![image](EdgePLC BL233说明书V1.1-images/image46.png)

### 2.4 LED

![image](EdgePLC BL233说明书V1.1-images/image45.jpeg)
...
![image](EdgePLC BL233说明书V1.1-images/image43.png)
...
![image](EdgePLC BL233说明书V1.1-images/image42.png)
...
![image](EdgePLC BL233说明书V1.1-images/image41.png)

此时LINK灯亮。

### 2.6 USB接口

![image](EdgePLC BL233说明书V1.1-images/image40.png)
...
![image](EdgePLC BL233说明书V1.1-images/image39.png)
...
![image](EdgePLC BL233说明书V1.1-images/image38.png)
...
![image](EdgePLC BL233说明书V1.1-images/image37.png)
...
![image](EdgePLC BL233说明书V1.1-images/image36.png)

### 2.7 HDMI接口

![image](EdgePLC BL233说明书V1.1-images/image35.png)
...
![image](EdgePLC BL233说明书V1.1-images/image34.png)

调试接口如图。可通过该端口进入设备系统。

### 2.9 SIM卡插槽

![image](EdgePLC BL233说明书V1.1-images/image32.png)

SIM卡槽如图所示。

### 2.10 SD卡插槽

![image](EdgePLC BL233说明书V1.1-images/image31.png)
...
![image](EdgePLC BL233说明书V1.1-images/image30.png)

系统识别到存储介质包含两个有效大分区，后续读写性能测试将限定在SD卡路径下进行”，并且可直观读取到该存储设备的实际物理容量。

然后输入以下指令测试写入：

fio --filename=/dev/mmcblk1p1 --ioengine=psync --rw=write --bs=1024k --size=1G --numjobs=1 --thread --group_reporting --name=write_job --ramp_time=1 --direct=1

可以看到工作状态下的（负载）写入的速度。

![image](EdgePLC BL233说明书V1.1-images/image29.png)
...
fio --filename=/dev/mmcblk1p1 --ioengine=psync --rw=read --bs=1024k --size=1G --numjobs=1 --thread --group_reporting --name=read_job --ramp_time=1 --direct=1

可以看到工作状态下读取的速度。

![image](EdgePLC BL233说明书V1.1-images/image28.png)
...
![image](EdgePLC BL233说明书V1.1-images/image27.png)
...
![image](EdgePLC BL233说明书V1.1-images/image26.png)

stty -F /dev/ttyUSB1 ispeed 115200 ospeed 115200 cs8 raw -echo //设置串口

![image](EdgePLC BL233说明书V1.1-images/image25.png)

查信号 20以上 ：

cat /dev/ttyUSB1 & echo -e "AT+CSQ\r" > /dev/ttyUSB1（输入两遍）

![image](EdgePLC BL233说明书V1.1-images/image24.png)

查是4G模块否能正常和SIM卡通讯：

echo -e "AT+CPIN?\r" > /dev/ttyUSB1

![image](EdgePLC BL233说明书V1.1-images/image23.png)
...
![image](EdgePLC BL233说明书V1.1-images/image21.png)
...
![image](EdgePLC BL233说明书V1.1-images/image20.png)
...
![image](EdgePLC BL233说明书V1.1-images/image19.png)
...
![image](EdgePLC BL233说明书V1.1-images/image18.png)
...
![image](EdgePLC BL233说明书V1.1-images/image17.png)

ifconfig #查看wlan0有无IP

![image](EdgePLC BL233说明书V1.1-images/image16.png)

最后ping百度测试：

ping www.baidu.com （百度域名不一定成功转IP，使用百度IP）

![image](EdgePLC BL233说明书V1.1-images/image15.png)
...
![image](EdgePLC BL233说明书V1.1-images/image14.png)
...
![image](EdgePLC BL233说明书V1.1-images/image13.png)

### 3.2 SSH2登录

在使用网口登录前需设置对应网口的IP。此处以ETH2为例。此处ETH2已连接路由器，获取到的IP为192.168.2.107。电脑IP在2网段。

![image](EdgePLC BL233说明书V1.1-images/image12.png)

点击创建连接，选择协议为SSH2，主机名填写为设备IP：192.168.2.107，端口22，用户名root，点击Connect连接。

![image](EdgePLC BL233说明书V1.1-images/image10.png)

选择接受，即连接成功。

![image](EdgePLC BL233说明书V1.1-images/image9.png)
...
![image](EdgePLC BL233说明书V1.1-images/image8.png)

工具运行后会自动识别接入到PC端的Micro SD卡，如下图所示。

![image](EdgePLC BL233说明书V1.1-images/image7.png)

将系统镜像拷贝至非中文路径的目录下。在PhoenixCard软件上点击“固件”选择目标镜像文件，接着选择“启动卡”，最后点击“烧卡”制作“启动卡”

![image](EdgePLC BL233说明书V1.1-images/image6.png)

系统启动卡制作完成后，状态栏呈现绿色并打印如下信息。

![image](EdgePLC BL233说明书V1.1-images/image5.png)
...
![image](EdgePLC BL233说明书V1.1-images/image4.png)
...
![image](EdgePLC BL233说明书V1.1-images/image3.png)

“量产卡”制作完成后，状态栏将会打印如下信息。

![image](EdgePLC BL233说明书V1.1-images/image2.png)

#### 4.2.2系统烧录

将制作好的SD卡插至设备Micro SD卡槽，上电后将从SD卡启动，并自动固化系统至eMMC中。当系统固化完成后，设备将自动掉电。串口打印如下。

![image](EdgePLC BL233说明书V1.1-images/image1.png)
...
![image](EdgePLC BL233说明书V1.1-images/image97.png)
...
配备导轨卡扣分为下段式短款导轨。此设计预留了充足的螺丝锁付空间，便于用户进行安装操作。

![image](EdgePLC BL233说明书V1.1-images/image96.png)
![image](EdgePLC BL233说明书V1.1-images/image95.png)

将卡扣垂直向下按压，直至其完全嵌入并锁定于导轨底部，即可完成安装。长导轨卡无需额外卡扣，因其内部已预设专用凹槽结构，可直接实现稳固连接。

![image](EdgePLC BL233说明书V1.1-images/image94.png)
...
NexPLC等软件平台将持续迭代更新。如需获取进一步的技术支持或了解后续规划，欢迎随时与我们联系。
